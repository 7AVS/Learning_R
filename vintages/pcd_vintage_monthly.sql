...
-- Cohort bin: calendar month 'YYYYMM' of a deployment's own response_start
...
--   DROP TABLE vt_pcd_monthly_cells;
--   DROP TABLE vt_pcd_monthly_spine;

-- ============================================================================
-- STEP 1: denominator cells
-- ============================================================================
CREATE VOLATILE TABLE vt_pcd_monthly_cells AS (
    WITH bin_arm_lookup AS (
        SELECT
            clnt_no,
            CAST(
                CAST(EXTRACT(YEAR FROM response_start) AS VARCHAR(4)) ||
                CASE WHEN EXTRACT(MONTH FROM response_start) < 10 THEN '0' ELSE '' END ||
                CAST(EXTRACT(MONTH FROM response_start) AS VARCHAR(2))
...
COLLECT STATISTICS ON vt_pcd_monthly_cells COLUMN (cohort, arm);

-- ============================================================================
-- STEP 2: day spine 0-60
-- ============================================================================
CREATE VOLATILE TABLE vt_pcd_monthly_spine AS (
...
COLLECT STATISTICS ON vt_pcd_monthly_spine COLUMN (vintage_day);
...
dense_grid AS (
    ...
    FROM vt_pcd_monthly_cells c
    CROSS JOIN vt_pcd_monthly_spine s
)
