...
-- Cohort bin: CALENDAR quarter 'YYYYQn' (Jan-Mar=Q1) of a deployment's own offer_start_date
...
--          acct_no) on this deduped set. Quarterly cohort_size <= sum of the 3 monthly
--          cohort_sizes — the gap is clients contacted in more than one month of the quarter.
...
--          clients — this sums cleanly: quarterly cum_responses = sum of the 3 monthly files'
--          cum_responses (each success belongs to exactly one deployment -> one month -> one
--          quarter).
    ...
    WITH bin_arm_lookup AS (
        SELECT
            ...
            CAST(
                ...
                CASE
                    WHEN EXTRACT(MONTH FROM offer_start_date) IN (1,2,3)    THEN 'Q1'
                    WHEN EXTRACT(MONTH FROM offer_start_date) IN (4,5,6)    THEN 'Q2'
                    WHEN EXTRACT(MONTH FROM offer_start_date) IN (7,8,9)    THEN 'Q3'
                    WHEN EXTRACT(MONTH FROM offer_start_date) IN (10,11,12) THEN 'Q4'
                END
...
COLLECT STATISTICS ON vt_crv_quarterly_cells COLUMN (cohort, arm);
...
CREATE VOLATILE TABLE vt_crv_quarterly_spine AS (
    ...
    WHERE (calendar_date - DATE '1900-01-01')
          BETWEEN 0 AND (SELECT MAX(cohort_max_day) FROM vt_crv_quarterly_cells)
...
dense_grid AS (
    ...
    CROSS JOIN vt_crv_quarterly_spine s
