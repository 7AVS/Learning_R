...
--             original campaigns/VBA_VBU/vba_vintage_curves.sql; confirmed instead in the
--             sibling harness campaigns/VBA_VBU/vba_summary_vintage_cell.py (`tc()`, ~line 68)
--             as VBA/VBU's real Test/Control rollup rule.
...
--             reported by both systems, done BEFORE deployment attribution.
-- STRUCTURAL CHANGE from vba_vintage_monthly.sql: the old file had no deployment output
...
COLLECT STATISTICS ON vt_vba_cells COLUMN (cohort_month, deployment, grp);
