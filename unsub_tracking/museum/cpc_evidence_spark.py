...
# %% [7b] SIZING DIAGNOSTIC - decides dump-raw vs join-server-side per table, on measured rows not estimates.
# Windows per Andre 2026-07-24: CPC + both vendor tables >= 2024-01-01, TACTIC >= 2025-06-01.
# CAL = timed 100k pull per table (measures the actual pipe: Teradata -> socket -> pandas) -> rows/s + bytes/row.
# est_pull_min is TRANSFER ONLY - server scan/spool + TDWM queue come on top. Expect the whole cell to run 5-15 min.
import time
t_cell0 = time.time()
...
print("diagnostic cell total: %.1f min" % ((time.time() - t_cell0) / 60.0))
