...
# %% [13] Q3d - STATE GRID: one row per (before, after, writer, SF flag) combination.
# Every waterfall bar = SUM of rows. Label in Excel; bar_v3 is a default only.
sql_q3d = """
WITH u_a AS (
...
"""
df_q3d = edw_query(sql_q3d)
print("Q3d rows:", len(df_q3d), "| clients:", int(df_q3d["clients"].sum()))
display(df_q3d)
display(df_q3d.groupby("bar_v3", as_index=False)["clients"].sum())

# %% [14] Q3e - client list for the 473,863 hatched-bar clients (active, 1012 open, SF unsub). No cap.
sql_q3e = """
WITH u_b AS (
    SELECT DISTINCT CLNT_NO FROM DDWV01.RB_CLNT_DLY
    WHERE SNAP_DT = DATE '2026-07-31' AND CLNT_STS = 'A'
...
"""
df_q3e = edw_query(sql_q3e)
print("Q3e rows:", len(df_q3e), "(expect 473,863)")
display(df_q3e)

# %% [15] Q3f - probe: PREF_ID x writer x consent at Jul-26 for the 473,863 hatched clients.
sql_q3f = """
WITH u_b AS (
    SELECT DISTINCT CLNT_NO FROM DDWV01.RB_CLNT_DLY
    WHERE SNAP_DT = DATE '2026-07-31' AND CLNT_STS = 'A'
...
ORDER BY 1,2,3
"""
df_q3f = edw_query(sql_q3f)
display(df_q3f)

# %% [16] Q3g - Q3e + other gates pivoted (1046 columns + counts). 1 row per client.
sql_q3g = """
WITH u_b AS (
    SELECT DISTINCT CLNT_NO FROM DDWV01.RB_CLNT_DLY
    WHERE SNAP_DT = DATE '2026-07-31' AND CLNT_STS = 'A'
...
"""
df_q3g = edw_query(sql_q3g)
print("Q3g rows:", len(df_q3g), "(expect 473,863) | 1046 closed:", int((df_q3g["cons_1046_jul26"]==5002).sum()),
      "| any other gate closed by 7020:", int((df_q3g["n_other_gates_closed_by_7020"]>0).sum()))
display(df_q3g)
