...
4. **Volatile Tables Consume Spool (2026-09-04, unsub pack 54):** volatile tables are stored in the user's spool space. A 110M-row volatile table (all Cards decisions since 2024, every channel) exhausted spool, so every later step failed "no more spool". **Rules**: keep volatile tables to the columns and rows the join needs (filter arms/channels/window BEFORE materializing); never write `col IN (SELECT key FROM big_volatile)` — materialize a tiny DISTINCT-key driver table with stats and INNER JOIN to it; add a 3-row size check after the first volatile step; NO `DROP TABLE` statements inside packs (2026-09-04, Andre: DROP errors made it impossible to tell whether a run worked). All drops live in `unsub_tracking/archaeology/00_reset_volatiles.sql`, run only before a same-session rerun. Previous packs' volatile tables still eat spool, so the reset file must list every pack's tables; never build a 3-way join (MASTER × EVENT × driver) in one CREATE — materialize each side, join the two small tables, drop the intermediates.
