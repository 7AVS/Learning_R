...
# --- Results collector ---
eda_results = {}  # key = section name, value = DataFrame
...
try:
    ...
    print("Warning: could not create HDFS output directory. Will try saving anyway.")
...
eda_results["PCD_row_count"] = pd.DataFrame([{"table": "PCD", "row_count": pcd_count}])
...
eda_results["PCD_date_ranges"] = pd.DataFrame([{"field": label, "min": mn, "max": mx} for label, mn, mx in [("response_start", r[0], r[1]), ("response_end", r[2], r[3]), ("dt_prod_change", r[4], r[5]), ("success_dt_1", r[6], r[7]), ("success_dt_2", r[8], r[9])]])
...
eda_results["PCD_null_pct"] = pcd_nulls
...
for col in ["mnemonic", "product_at_decision", "product_grouping_at_decision",
    ...
    top = df.head(20).copy()
    top.insert(0, "field", col)
    top.columns = ["field", "value", "cnt", "pct"]
    pcd_cat_frames.append(top)
eda_results["PCD_categoricals"] = pd.concat(pcd_cat_frames, ignore_index=True)
...
pcd_num_df = pd.DataFrame([{"column": num_cols[i], "min": r[i*3], "max": r[i*3+1], "avg": r[i*3+2]} for i in range(len(num_cols))])
print(pcd_num_df.to_string(index=False))
eda_results["PCD_numeric_stats"] = pcd_num_df
...
for col in ["channel_deploy_cc", "channel_deploy_dm", "channel_deploy_do",
    ...
    top.insert(0, "field", col)
    top.columns = ["field", "value", "cnt", "pct"]
    pcd_chdeploy_frames.append(top)
eda_results["PCD_channel_deploy"] = pd.concat(pcd_chdeploy_frames, ignore_index=True)
...
eda_results["PCD_response_rates"] = pd.DataFrame([r], columns=["resp_any_pct", "resp_target_pct", "resp_upgrade_pct"])
...
r = cursor.fetchall()[0]
cursor.close()
pcd_oando_df = pd.DataFrame([r], columns=["oando_pct", "actioned_pct", "pending_pct", "declined_pct", "approved_pct"])
print(pcd_oando_df.to_string(index=False))
eda_results["PCD_oando_funnel"] = pcd_oando_df
...
r = cursor.fetchall()[0]
cursor.close()
pcd_email_df = pd.DataFrame([r], columns=["tactic_email_pct", "email_status_pct"])
print(pcd_email_df.to_string(index=False))
eda_results["PCD_email_rates"] = pcd_email_df
...
eda_results["PCD_email_disposition"] = df
...
r = cursor.fetchall()[0]
cursor.close()
pcd_digital_df = pd.DataFrame([r], columns=["olb_pct", "mb_pct", "dor_pct", "ss_act_pct", "ss_opn_pct"])
print(pcd_digital_df.to_string(index=False))
eda_results["PCD_digital_flags"] = pcd_digital_df
...
pcd_olb_df = pd.DataFrame([{"imp_olb_min": r[0], "imp_olb_max": r[1], "imp_olb_avg": r[2],
                      "clk_olb_min": r[3], "clk_olb_max": r[4], "clk_olb_avg": r[5]}])
print(pcd_olb_df.to_string(index=False))
eda_results["PCD_olb_impressions"] = pcd_olb_df
...
for col in ["invitation_to_upgrade", "target_product", "target_product_name",
    ...
    top.insert(0, "field", col)
    top.columns = ["field", "value", "cnt", "pct"]
    pcd_offer_frames.append(top)
eda_results["PCD_offers_upgrades"] = pd.concat(pcd_offer_frames, ignore_index=True)
...
cursor.close()
print(f"\nPLI: {pli_count:,} rows")
eda_results["PLI_row_count"] = pd.DataFrame([{"table": "PLI", "row_count": pli_count}])
...
for label, mn, mx in [("decision_dt", r[0], r[1]), ("actual_strt_dt", r[2], r[3]),
    ...
    print(f"  {label}: {mn}  to  {mx}")
eda_results["PLI_date_ranges"] = pd.DataFrame([{"field": label, "min": mn, "max": mx} for label, mn, mx in [("decision_dt", r[0], r[1]), ("actual_strt_dt", r[2], r[3]), ("parent_actual_strt_dt", r[4], r[5]), ("treatmt_strt_dt", r[6], r[7]), ("treatmt_end_dt", r[8], r[9]), ("dt_cl_change", r[10], r[11]), ("spid_proc_dt", r[12], r[13]), ("dt_acct_open", r[14], r[15])]])
...
eda_results["PLI_null_pct"] = pli_nulls
...
for col in ["increase_decrease", "product_current", "product_name_current",
    ...
    top.insert(0, "field", col)
    top.columns = ["field", "value", "cnt", "pct"]
    pli_cat_frames.append(top)
eda_results["PLI_categoricals"] = pd.concat(pli_cat_frames, ignore_index=True)
...
pli_num_df = pd.DataFrame([{"column": pli_num_cols[i], "min": r[i*3], "max": r[i*3+1], "avg": r[i*3+2]} for i in range(len(pli_num_cols))])
print(pli_num_df.to_string(index=False))
eda_results["PLI_numeric_stats"] = pli_num_df
...
pli_ch_df = pd.DataFrame([{"channel": ch_labels[i], "rate_pct": r[i]} for i in range(len(ch_labels))])
print(pli_ch_df.to_string(index=False))
eda_results["PLI_channel_rates"] = pli_ch_df
...
for col in ["channel", "response_channel", "response_source"]:
    ...
    top.insert(0, "field", col)
    top.columns = ["field", "value", "cnt", "pct"]
    pli_chcat_frames.append(top)
eda_results["PLI_channel_categoricals"] = pd.concat(pli_chcat_frames, ignore_index=True)
...
r = cursor.fetchall()[0]
cursor.close()
pli_flags_df = pd.DataFrame([r], columns=["responder_cli_pct", "decisioned_pct", "student_pct"])
print(pli_flags_df.to_string(index=False))
eda_results["PLI_key_flags"] = pli_flags_df
...
r = cursor.fetchall()[0]
cursor.close()
pli_oando_df = pd.DataFrame([r], columns=["oando_pct", "actioned_pct", "pending_pct", "declined_pct", "approved_pct"])
print(pli_oando_df.to_string(index=False))
eda_results["PLI_oando_funnel"] = pli_oando_df
...
r = cursor.fetchall()[0]
cursor.close()
pli_email_df = pd.DataFrame([r], columns=["tactic_email_pct", "email_status_pct"])
print(pli_email_df.to_string(index=False))
eda_results["PLI_email_rates"] = pli_email_df
...
print("\n  >> email_disposition:")
print(df.to_string(index=False))
eda_results["PLI_email_disposition"] = df
...
r = cursor.fetchall()[0]
cursor.close()
pli_digflags_df = pd.DataFrame([r], columns=["olb_pct", "mb_pct"])
print(pli_digflags_df.to_string(index=False))
eda_results["PLI_digital_flags"] = pli_digflags_df
...
pli_digstats_df = pd.DataFrame([{"column": dig_cols[i], "min": r[i*3], "max": r[i*3+1], "avg": r[i*3+2]} for i in range(len(dig_cols))])
print(pli_digstats_df.to_string(index=False))
eda_results["PLI_digital_stats"] = pli_digstats_df
...
for col in ["spid", "spid_label", "decile", "new_decile"]:
    ...
    top.insert(0, "field", col)
    top.columns = ["field", "value", "cnt", "pct"]
    pli_spid_frames.append(top)
eda_results["PLI_spid_model"] = pd.concat(pli_spid_frames, ignore_index=True)
...
cursor.close()
print("\n  Client type flags:")
pli_client_df = pd.DataFrame([r], columns=["pb_client_pct", "premier_pct", "tsne_pct"])
print(pli_client_df.to_string(index=False))
eda_results["PLI_client_flags"] = pli_client_df
...
eda_results["TPA_row_count"] = pd.DataFrame([{"table": "TPA", "row_count": tpa_count}])
...
for label, mn, mx in [("report_dt", r[0], r[1]), ("treatmt_start_dt", r[2], r[3]),
    ...
    print(f"  {label}: {mn}  to  {mx}")
eda_results["TPA_date_ranges"] = pd.DataFrame([{"field": label, "min": mn, "max": mx} for label, mn, mx in [("report_dt", r[0], r[1]), ("treatmt_start_dt", r[2], r[3]), ("treatmt_end_dt", r[4], r[5]), ("response_dt", r[6], r[7])]])
...
eda_results["TPA_null_pct"] = tpa_nulls
...
for col in ["mnemonic", "target_seg", "tpa_ita", "like_for_like_group",
    ...
    top.insert(0, "field", col)
    top.columns = ["field", "value", "cnt", "pct"]
    tpa_cat_frames.append(top)
eda_results["TPA_categoricals"] = pd.concat(tpa_cat_frames, ignore_index=True)
...
tpa_num_df = pd.DataFrame([{"column": tpa_num_cols[i], "min": r[i*3], "max": r[i*3+1], "avg": r[i*3+2]} for i in range(len(tpa_num_cols))])
print(tpa_num_df.to_string(index=False))
eda_results["TPA_numeric_stats"] = tpa_num_df
...
r = cursor.fetchall()[0]
cursor.close()
tpa_app_df = pd.DataFrame([r], columns=["app_completed_pct", "app_approved_pct"])
print(tpa_app_df.to_string(index=False))
eda_results["TPA_app_funnel"] = tpa_app_df
...
tpa_chrates_df = pd.DataFrame([{"channel": tpa_ch[i], "rate_pct": r[i]} for i in range(len(tpa_ch))])
print(tpa_chrates_df.to_string(index=False))
eda_results["TPA_channel_rates"] = tpa_chrates_df
...
print("\n  >> channel (text):")
print(df.to_string(index=False))
eda_results["TPA_channel_text"] = df
...
r = cursor.fetchall()[0]
cursor.close()
tpa_oando_df = pd.DataFrame([r], columns=["oando_pct", "actioned_pct", "pending_pct", "declined_pct", "approved_pct"])
print(tpa_oando_df.to_string(index=False))
eda_results["TPA_oando_funnel"] = tpa_oando_df
...
r = cursor.fetchall()[0]
cursor.close()
tpa_email_df = pd.DataFrame([r], columns=["tactic_email_pct", "email_status_pct"])
print(tpa_email_df.to_string(index=False))
eda_results["TPA_email_rates"] = tpa_email_df
...
print("\n  >> email_disposition:")
print(df.to_string(index=False))
eda_results["TPA_email_disposition"] = df
...
tpa_digstats_df = pd.DataFrame([{"column": tpa_dig[i], "min": r[i*3], "max": r[i*3+1], "avg": r[i*3+2]} for i in range(len(tpa_dig))])
print(tpa_digstats_df.to_string(index=False))
eda_results["TPA_digital_stats"] = tpa_digstats_df
...
r = cursor.fetchall()[0]
cursor.close()
tpa_cc_df = pd.DataFrame([r], columns=["tactic_call_pct", "contact_attempt_pct", "call_answered_pct", "agent_pct", "rpc_pct"])
print(tpa_cc_df.to_string(index=False))
eda_results["TPA_call_center"] = tpa_cc_df
...
print("\n  >> decsn_month:")
print(df.to_string(index=False))
eda_results["TPA_decsn_month"] = df
...
for label, tbl in [("PCD", PCD), ("PLI", PLI), ("TPA", TPA)]:
    ...
    eda_results[f"{label}_sample_rows"] = sample_df
...
eda_results["PCD_duplicates"] = pcd_dup
...
if len(pli_dup) == 1 and int(pli_dup.iloc[0]["rows_per_key"]) == 1:
    ...
    print("  => DUPLICATES FOUND on primary index. Investigate.")
eda_results["PLI_duplicates"] = pli_dup
...
if len(tpa_dup) == 1 and int(tpa_dup.iloc[0]["rows_per_key"]) == 1:
    ...
    print("  => DUPLICATES FOUND on primary index. Investigate.")
eda_results["TPA_duplicates"] = tpa_dup
...
recency_rows.append({"table": "PCD", "field_1": "response_start", "max_1": str(r[0]), "field_2": "response_end", "max_2": str(r[1])})
...
recency_rows.append({"table": "PLI", "field_1": "decision_dt", "max_1": str(r[0]), "field_2": "actual_strt_dt", "max_2": str(r[1])})
...
recency_rows.append({"table": "TPA", "field_1": "report_dt", "max_1": str(r[0]), "field_2": "treatmt_start_dt", "max_2": str(r[1])})

eda_results["data_recency"] = pd.DataFrame(recency_rows)
...
eda_results["row_count_summary"] = pd.DataFrame([
    {"table": "PCD Ongoing", "row_count": pcd_count},
    {"table": "PLI", "row_count": pli_count},
    {"table": "TPA PCQ", "row_count": tpa_count},
    {"table": "Total", "row_count": pcd_count + pli_count + tpa_count}
])


# ===================================================================
# Section 5: Save Results to HDFS (HTML + CSV)
# ===================================================================
...
html_parts.append(f"<h1>Cards Decision &amp; Response — EDA Report</h1>")
html_parts.append(f"<p class='timestamp'>Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>")

# Group results by table prefix
for table_prefix, table_label in [("PCD", "PCD Ongoing Decision Response"),
                                    ("PLI", "PLI Decision Response"),
                                    ("TPA", "TPA PCQ Decision Response"),
                                    ("data_recency", "Data Recency"),
                                    ("row_count", "Row Count Summary")]:
    matching = {k: v for k, v in eda_results.items() if k.startswith(table_prefix)}
    if matching:
        html_parts.append(f"<div class='section'><h1>{table_label}</h1>")
        for name, df in matching.items():
            clean_name = name.replace("_", " ").replace(table_prefix + " ", "")
            html_parts.append(f"<h2>{clean_name}</h2>")
...
# Save HTML to HDFS — IMPORTANT: path must end with .html
html_hdfs_path = f"{HDFS_OUT}/cards_eda_report.html"
try:
    # Write locally first, then copy to HDFS (saveAsTextFile creates a directory, not a file)
    local_html = "/tmp/cards_eda_report.html"
...
print(f"\n=== EDA Complete — {len(eda_results)} tables saved ===")
