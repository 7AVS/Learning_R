...
-- Dimensions: arm (challenger vs champion) x slicer (overall + strategy BAU/NTC) x metric x vintage_day.
--   arm is PERMANENT (challenger = served, champion = holdout) - conversion challenger vs champion = the lift.
...
base AS (                                             -- slicer long-format; arm carried as permanent column
  SELECT clnt_no, arm, strategy, responder_cli, dt_cl_change, treatmt_strt_dt,
...
events AS (                                            -- per-client first-event vintage_day, per metric
  SELECT b.arm, b.slicer_dim, b.slicer_value, 'engagement' AS metric, b.clnt_no,
  ...
  UNION ALL
  SELECT b.arm, b.slicer_dim, b.slicer_value, 'conversion' AS metric, b.clnt_no,
...
base_n AS (                                            -- denominator per arm x slice
  SELECT arm, slicer_dim, slicer_value, COUNT(*) AS base_clients
  FROM base GROUP BY arm, slicer_dim, slicer_value
),
daily AS (                                             -- daily first-event counts
  SELECT arm, slicer_dim, slicer_value, metric, vintage_day, COUNT(DISTINCT clnt_no) AS n_events
  FROM events WHERE vintage_day >= 0
  GROUP BY arm, slicer_dim, slicer_value, metric, vintage_day
),
spine AS (                                             -- day spine 0..max
  SELECT s AS vintage_day
  FROM UNNEST(sequence(0, (SELECT MAX(vintage_day) FROM daily))) AS t(s)
),
grid AS (                                              -- dense arm x slice x metric x day
  SELECT d.arm, d.slicer_dim, d.slicer_value, d.metric, sp.vintage_day
  FROM (SELECT DISTINCT arm, slicer_dim, slicer_value, metric FROM daily) d
  CROSS JOIN spine sp
)
SELECT
  g.arm,
  ...
  SUM(COALESCE(dl.n_events, 0)) OVER (
    PARTITION BY g.arm, g.slicer_dim, g.slicer_value, g.metric
    ORDER BY g.vintage_day ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
  ) AS cum_clients
FROM grid g
LEFT JOIN daily dl
  ON dl.arm = g.arm AND dl.slicer_dim = g.slicer_dim AND dl.slicer_value = g.slicer_value
 AND dl.metric = g.metric AND dl.vintage_day = g.vintage_day
JOIN base_n bn ON bn.arm = g.arm AND bn.slicer_dim = g.slicer_dim AND bn.slicer_value = g.slicer_value
ORDER BY g.metric, g.arm, g.slicer_dim, g.slicer_value, g.vintage_day;
