...
================================================================
## 2026-06-15 — Q28 channel re-run: CRV/PCL click labels CORRECTED + bug-vs-conversion test logic
================================================================

### A. CORRECTED Q28 channel assignment (labels disambiguated via control arm)

New labeled run output (Action·Both arm, by platform):

  overlap_arm     bucket  platform  crv_viewers  crv_click_clients  crv_ctr  pcl_viewers  pcl_click_clients  pcl_ctr
  overlap_action  Both    ANDROID   103,393      21,430 (21%)       —        102,756      45,199 (44%)       —
  overlap_action  Both    iOS       ~393K        ~67,175 (17%)      —        ~396K        ~130,326 (33%)     —

  click_crv_only (Android) = 2,681   |   click_pcl_only (Android) = 26,450
  In the Both bucket: PCL is clicked MORE than CRV on both platforms (~2:1 ratio).

Control arm confirms label direction:
  overlap_control / PCL-only arm: crv_click_clients = 0, pcl_click_clients > 0
  (e.g. iOS: crv_click = 0, pcl_click = 9,764). Labels are correct.

### B. MISLABEL FLAG — existing findings vs this run

findings.md had NO prior Q28 section (file ended at Q05 / Session end 2026-05-26).
However, the channel_bulletproofing RESULTS_CATALOG and Q20 context referenced
a "PCL click 21.1% Action" figure. That 21% corresponds to the crv_click_clients
column in the new labeled output (~21% in Action·Both·Android), NOT to PCL.
The real PCL click rate in Action·Both is ~44% (Android) / ~33% (iOS).

STATUS: potential label swap in any prior channel-click output citing ~21% as PCL.
NEEDS RECONCILING before quoting any channel-click rate externally.
This potentially reverses any documented "PCL clicked LESS in Action" direction —
flag for review, do NOT silently trust old figures.

### C. Channel click-cannibalization is NOT robust across platforms

  Android  PCL click Action·Both = 44%   vs  Control·Both = 35%   → Action HIGHER (no cannibalization)
  iOS      PCL click Action·Both = 33%   vs  Control·Both = 41%   → Action lower by ~8pp

The only platform showing a PCL click dip (iOS) is the platform with the known
mobile-priority serving bug (CRV under-served relative to intent on iOS).
The iOS click dip is more plausibly the serving-bug footprint than cannibalization.

CONCLUSION: channel click-cannibalization should NOT be treated as an established finding.
The 1.08pp conversion gap (Q04) remains the headline — channel is internal diagnostic only.

### D. Bug-vs-conversion test logic

The iOS serving bug is an engagement/serving issue (upstream of click).
The 1.08pp conversion gap is a randomized Action-vs-Control result (outcome layer).
The bug cannot by itself explain the conversion gap — different funnel layers.

Existing robustness already argues against the bug as cause:
- Q09 (gap order-independent, stable ~1pp regardless of timing within window)
- Q12 (no dose-response; gap flat vs overlap length)
Both are invariant to exactly what the serving bug changes (exposure amount, serving order).

Confirmatory test design:
  Fix: restore intended iOS serving priority → keep measuring Action-vs-Control PCL response.
  Interpretation asymmetry:
    Gap PERSISTS post-fix (~1pp) = conversion cannibalization confirmed (independent of channel)
    Gap SHRINKS = the cure working (overlap reduced) — NOT disproof of cannibalization
  Pre-register: define window + "gap holds ~1pp" success criterion BEFORE the fix is applied.

Hard constraint reaffirmed: channel stays OUT of decks and stakeholder comms.
This section is internal diagnostic only.
