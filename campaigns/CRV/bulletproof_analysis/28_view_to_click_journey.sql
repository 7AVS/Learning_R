...
-- Cohort: Dec 2025-Feb 2026 (fully-observed; standalone cohort, distinct from the Feb-Apr headline).
...
ga4_events AS (
    ...
    WHERE event_date >= DATE '2025-12-01'
      AND ((year = '2025' AND month IN ('12')) OR (year = '2026' AND month IN ('01','02','03','04','05')))  -- partition prune: PCL deployments START Dec 2025-Feb 2026; their ~90-day windows reach into May 2026 (per-deployment BETWEEN join keeps only in-window events).
...
-- Funnel read: clients = viewers in this view_category bucket; crv_click_clients/pcl_click_clients = how many of those viewers clicked each banner.
SELECT
    overlap_status,
    ...
    platform,
    COUNT(*)                                                                          AS clients,
    SUM(CASE WHEN crv_click = 1 THEN 1 ELSE 0 END)                                   AS crv_click_clients,
    SUM(CASE WHEN pcl_click = 1 THEN 1 ELSE 0 END)                                   AS pcl_click_clients,
...
-- ===== ALT OUTPUT: per-OS view+click funnel — explicit symmetric counts by overlap_status × platform (run separately) =====
-- Collapses view_category detail; shows absolute view and click totals side-by-side for each OS arm.
WITH
crv_action AS (
    SELECT acct_no, offer_start_date, offer_end_date
    FROM dl_mr_prod.cards_crv_install_decis_resp
...
SELECT
    ...
    SUM(CASE WHEN crv_view  = 1 THEN 1 ELSE 0 END)                   AS crv_view_clients,
    SUM(CASE WHEN crv_click = 1 THEN 1 ELSE 0 END)                   AS crv_click_clients,
    SUM(CASE WHEN pcl_view  = 1 THEN 1 ELSE 0 END)                   AS pcl_view_clients,
    SUM(CASE WHEN pcl_click = 1 THEN 1 ELSE 0 END)                   AS pcl_click_clients
FROM client_roll
GROUP BY overlap_status, platform
ORDER BY overlap_status, platform
;

-- ===== ALL JOURNEYS (long format): one row per viewed-state -> clicked-state path, distinct clients (run separately) =====
WITH
crv_action AS (
    SELECT acct_no, offer_start_date, offer_end_date
    FROM dl_mr_prod.cards_crv_install_decis_resp
...
SELECT
    overlap_status,
    platform,
    CASE WHEN crv_view = 1 AND pcl_view = 1 THEN 'Viewed both'
         WHEN crv_view = 1 AND pcl_view = 0 THEN 'Viewed CRV only'
         WHEN crv_view = 0 AND pcl_view = 1 THEN 'Viewed PCL only'
         ELSE                                    'Viewed neither' END AS viewed,
    CASE WHEN crv_click = 1 AND pcl_click = 1 THEN 'Clicked both'
         WHEN crv_click = 1 AND pcl_click = 0 THEN 'Clicked CRV only'
         WHEN crv_click = 0 AND pcl_click = 1 THEN 'Clicked PCL only'
         ELSE                                      'Clicked neither' END AS clicked,
    COUNT(*) AS clients
FROM client_roll
GROUP BY 1, 2, 3, 4
ORDER BY 1, 2, 3, 4
