...
-- Q24 — PCL CONTACT FREQUENCY x CRV OVERLAP STATUS (Dec 2025-Feb 2026, two statements)
-- Cohort shifted to Dec 2025-Feb 2026 deployments (from Feb-Apr 2026) so 90-day engagement
--   windows are fully observed within GA4 data through May 2026.
-- NOTE: this makes Q24 a standalone contact-frequency cohort, NOT the same population as
--   the Feb-Apr headline gap.
-- Frequency buckets UNCAPPED (raw integers, no '+' collapse).
-- Statement 1: contact frequency = CUMULATIVE touch number over the FULL 20-month
--   history (Oct-2024+, Q11 crv_touch_number convention) read at the Dec 2025-Feb 2026
--   measured deployments — "how many times contacted by the time we measured them"
--   (raw integer 1..N) — plus engagement overlay: viewed / clicked / view-days in-window.
-- Statement 2: CHANNEL-side contact frequency — clients by number of deployments where
--   the banner actually reached them (raw integer 0..N, ~= months seen), clicks + converters per bucket.
...
-- CRV offer window: restricted to offer_end_date >= 2025-12-01 (matched to PCL Dec cohort
--   window); CRV offers ending before Dec 2025 are excluded → those clients classify as no_overlap.
...
-- Statement 1: contact frequency x engagement overlay (Dec 2025-Feb 2026 cohort)
...
pcl_universe AS (   -- measured leads = Dec 2025-Feb 2026, carrying their cumulative touch number
...
SELECT
    cf.overlap_status,
    cf.pcl_contacts_20mo                  AS pcl_contact_freq_20mo,
    COUNT(*)                          AS clients,
...
-- the frequency source: in how many of their Dec 2025-Feb 2026 deployments did the banner
-- actually reach the client (>=1 view inside that deployment's own window)? Clients bucketed
-- by banner-reached deployments (raw integer 0..N — one PCL deployment per month, so this
-- ~= months seen), with click interaction and converters per bucket.
...
pcl_universe AS (
    ...
    LEFT JOIN coapp_accts x
      ON x.acct_no = p.acct_no
    WHERE p.treatmt_strt_dt BETWEEN DATE '2025-12-01' AND DATE '2026-02-28'
      AND p.channel LIKE '%MB%'
      AND x.acct_no IS NULL
),
overlap_action_keys AS (
...
SELECT
    overlap_status,
    deployments_banner_seen                                AS banner_contact_freq,
    COUNT(*)                                               AS clients,
