-- PCL conversion + CRV conversion by PCL decile, split by overlap arm.
-- Uses the EXACT aggregation idioms from Q06 (the confirmed-working query): SUM of 0/1 flags
-- and SUM(CASE WHEN flag = 1 THEN responder_cli ELSE 0 END). Only the GROUP BY changed
-- (decile instead of month). No COUNT(*), no cohort label, no UNION -- the parts that differed
-- from Q06 and triggered the 2616/22003 overflow are gone.
-- Output is arm-columnar by decile: action-overlap vs control-overlap -> the cannibalization gap per decile.
-- (Slice is on pli decile from model_score. new_decile/no_overlap can be added once this runs.)
