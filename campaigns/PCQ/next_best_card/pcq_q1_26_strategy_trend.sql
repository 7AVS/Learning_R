...
-- Cumulative time buckets: 15 / 30 / 45 / 60 / 90 days post treatment
--   - approved/responded buckets = denominators (eligible by day N)
--   - purchase buckets           = numerators   (spent by day N)

DATABASE DL_MR_PROD;

-- ============================================================
-- Drop volatiles from any prior run in this session.
-- First-time run will throw "object does not exist" (Teradata 3807)
-- on each DROP -- that is expected; let it through and continue.
-- ============================================================
DROP TABLE pcq_q1_acct_summary;
DROP TABLE pcq_q1_approved;
DROP TABLE pcq_q1_cohort;
...
--   sum_purchases         = SUM(net_prch_amt_dly) total
--   sum_purchases_d0_N    = cumulative spend within first N days
...
-- contacted via that channel in the cohort.
-- d0_N suffixes = cumulative within first N days post treatment.
-- ============================================================
SELECT
  c.treatmt_start_dt,
  c.test_group_latest,
